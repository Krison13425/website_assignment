...
<html>
	...
	<br>
		...
		<?php
			...
			$sql = "SELECT * FROM staff WHERE Staff_type='Staff'";
			...
			if($queryResults > 0){
				while ($row = mysqli_fetch_assoc($result))
				{
					...
					echo'<td><button class="btn btn2" onclick="window.location.href=\'delete_staff.php?id='.$row['Staff_ID'].'\'">Delete</button></td>';
			}
			echo"</table>";
			}else{
				echo"0 result";
